...
`filename(if NULL, only changes mode), using mode (‘r’=read, ‘w’=(over)write, ‘a’=append, ‘r+’,’w+’,’a+’=with update)` :  
...
## SET
//elementi unici ordinati in modo crescente  
`#include <set>`  
*	auto set_name = setTYPE();  
*	auto set_name = setTYPE, DECLTYPE(ORDINE) (ordine);	//segue l’ordine scelto  
  
//WITH PAIRS  
```
struct comp{  
*	bool operator()(pairINT,INT a, pairINT,INT b){  
…  
setPAIRINT,INT,COMP set_name  
```
...
std::setTYPE…::iterator it_name = s.begin() / s.end();	poi ++/-- ....  
...
## PRIORITY_QUEUE  
`priority_queue <TYPE, VECTORTYPE(DEL SOTTOINSIEME), CLASS COMPARATOR>`   nome_queue;  
...
## UNORDERED_MAP
//dizionario (key-value, niente ordine)  
