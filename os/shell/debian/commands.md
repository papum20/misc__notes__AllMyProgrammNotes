...
cd / : root directory  
cd .. : parent directory  
cd -L : follow symbolic link  
cd - : to previous directory  
find <dir> : list (recursively) elements in dir  
-maxdepth levels : 	  
-mindepth levels :   
find <dir> -name <file_name> :  
find files (recursively) in dir matching basename with file_name  
...
-o : -l without groups  
mkdir <> :  
	mkdir -p <> : ignore errors; also make parent dirs if needed  
mv <> <> :  
pushd : cd, allows to go back  
pwd : current path  
rm : remove  
rm -r : recursive  
rm -f : force (also rm --force)   
...
FILES :   
cp <old_file> <new_file> : copy  
	cp <file1> <file2> <...> <dir> : copy all files in dir  
dd if=<from_path> of=<to_path> :  
